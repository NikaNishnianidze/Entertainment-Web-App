...
export default function BookMark() {
  ...
  const filteredMedia = mediaData.filter(
    (media) =>
      media.isBookmarked == true &&
      media.title.toLowerCase().includes(searchValue.toLowerCase())
  );
  const bookmarkedMovies = mediaData.filter(
    (media) => media.isBookmarked && media.category === "Movie"
  );
  const bookmarkedTvSeries = mediaData.filter(
    (media) => media.isBookmarked && media.category === "TV Series"
  );
  ...
  return (
    <div className="dk:pl-[164px]">
      ...
      {searchValue && filteredMedia.length ? (
        <div>
          ...
          <div className="recommended flex flex-row items-center justify-center tb:justify-start tb:pl-[25px] tb:gap-[29px] flex-wrap mb-[60px] px-[16px] gap-[15px] mt-[24px]">
            {filteredMedia.map((media) => {
              return (
                <div
                  key={media.title}
                  className="mt-[24px] w-[164px] mb-[16px] h-[110px] rounded-[8px] relative tb:w-[220px] tb:h-[140px]"
                >
                  <img
                    src={media.thumbnail.regular?.small}
                    alt="film thumbnails"
                    className="rounded-[8px] tb:w-[220px] tb:h-[140px]"
                  />
                  <div
        ...
        <div>
          <p className="mt-[26px] pl-[16px] text-[#fff] text-[20px] font-normal tb:pl-[25px] tb:mt-[34px] tb:text-[32px]">
            Bookmarked Movies
          </p>
          <div className="movies flex flex-row items-center justify-center flex-wrap mb-[60px] px-[16px] gap-[15px] mt-[24px]">
            {bookmarkedMovies.map((media) => {
              return (
                <div
                  key={media.title}
          ...
          <p className="mt-[24px] pl-[16px] text-[20px] text-[#fff] font-normal tb:mt-[34px] tb:pl-[25px] tb:text-[32px]">
            Bookmarked TV Series
          </p>
          <div className="series flex flex-row items-center justify-center flex-wrap mb-[60px] px-[16px] gap-[15px] mt-[24px]">
            {bookmarkedTvSeries.map((media) => {
              return (
                <div
                  key={media.title}
                  className="mt-[24px] w-[164px] mb-[16px] h-[110px] rounded-[8px] relative tb:w-[220px] tb:h-[140px] dk:w-[280px] dk:h-[174px]"
